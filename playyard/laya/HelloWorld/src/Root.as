package {
	...
	public class Root {
		...
		private function testNumber(): void {
			...
			// 对浮点数进行向上取整
			var numCeil: int = Math.ceil(num);
			console.log(num + '向上取整：' + numCeil);
			
			// 对浮点数进行向下取整
			var numFloor: int = Math.floor(num);
			console.log(num + '向下取整：' + numFloor);
		}
		
		private function testBoolean(): void {
			console.log('----------------Boolean----------------');	
			// 以下演示布尔值
			...
			// Math.random会随机返回一个0~1的浮点数
			var numA: Number = Math.random();
			var numB: Number = Math.random();
			var result: Boolean = numA < numB;
			if(result) {
