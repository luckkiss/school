...
function go(projPath, webPath, jsNames, jsonNames, workFolder, useUglyRecord, donotFormat, replaceIt, workMode) {
    var logContent = '';
    ...
    const layaResPath = webPath + 'res/Laya';
    const jsonPath = webPath + 'res/data';
    const cfgClassPath = projPath + 'src/automatic/cfgs'
    ...
    const confuseMode = workMode.indexOf('c') >= 0;
    const insertMode = workMode.indexOf('i') >= 0;
    
    log('run esprima, projPath = ' + projPath + ', webPath = ' + webPath + ', jsNames = ' + jsNames + ', jsonNames = ' + jsonNames + ', useUglyRecord = ' + useUglyRecord + ', donotFormat = ' + donotFormat + ', replaceIt = ' + replaceIt + ', workMode = ' + workMode);
    ...
    var noUglyMap = {};
    var savedIDMap = {};
    const noUglyShortest = 2;  // 长度超过2的才需要混淆
    ...
    log('fetching keywords from laya res files...');    
    var layakeys = [];
    ...
    log(layakeys.length + ' keywords from laya res files finished, ' + (et - st) + 'ms costed. ');
...
　　　　　　　　　　// 读取文件内容
    ...
    function insertSimpleVars(ast) {
        ...
        if(ast.type == 'BlockStatement' && insertCnt < MaxInsertCnt && Math.random() > 0.8) {
            ast.body.splice(Math.floor(Math.random() * ast.body.length), 0, genSimpleVar());
            insertCnt++;
        }
    }
    ...
    function processLiteral(ast) {
        if(typeof(ast.value) == 'string' && true !== cfgClassNameMap[ast.value] && !ast.__packageName) {
            // 字符串类型的不予混淆
        ...
        for(var i = 0, ltrLen = ltr.length; i < ltrLen; i++) {
            ...
            for(var j = 0, ltrnumLen = ltrnum.length; j < ltrnumLen; j++) {
                ...
                for(var k = 0, ltrnumLen = ltrnum.length; k < ltrnumLen; k++) {
                    var uk = u + ltrnum[k];
                    if(!protectedMap[uk] && !recordMap[u] && !savedIDMap[u]) {
                        uglyPool.push(uk);
                    }
                }
            }
    ...
    function codeFromLiteral(ast) {
        ...
        if(true === cfgClassNameMap[ast.value]) {
            return '\'' + toUgly(ast.value) + '\'';
        }
    ...
    function genSimpleVar() {
        var declarations = [];
        var dcnt = Math.ceil(Math.random() * 5);
        for(var i = 0; i < dcnt; i++) {
            var did = genIdentifier(0);
            var dinit = null;
            var rdn = Math.random();
            if(rdn > 0.75) {
    ...
    function genObjectExpression(deep, choices, excludes, properties) {
        if(!properties) {
            ...
            var pcnt = Math.ceil(Math.random() * 5);
            for(var i = 0; i < pcnt; i++) {
                var ppt = genProperty(deep + 1, choices, excludes);
                properties.push(ppt);
            }
        }
    ...
    function genProperty(deep, choices, excludes, key, value, kind) {
        if(!key){
            key = genIdentifier(deep + 1, choices, excludes);
        }
        if(!value) {
    ...
    function genSequenceExpression(deep, choices, excludes, expressions) {
        if(!expressions) {
            expressions = genExpressions(deep + 1, ["Identifier"], mergeArray(excludes, ['AssignmentExpression']));
        }
